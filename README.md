...
// Adding a method to the prototype chain
Person.prototype.greet = () => {
  console.log(`Hello, my name is ${this.name}`);
};

// Creating an instance of Person
const ann = new Person("Ann", 24);
// Calling the greet method (using the prototype chain)
ann.greet(); // Output: 'Hello, my name is undefined'
```

This shows the potential pitfall when working with prototype chain.

- The `Person.prototype.greet` doesnt bind its own `this`, which means it refers to the surrounding context instead of the instance `ann` leading to `this.name` being undefined.
...
// Create instance
const classmate = new Student("Al", 12, 18);

// Calling methods
classmate.greet(); //Output: Hey Al
classmate.study(); //Output: Al is 18
```

- The `class` syntax is used to define `School` and `Student` classes, which makes the code more readable.
- Inheritance is achieved using the `extends` keyword, making `Student` a subclass of `School`.
